...
if __name__ == "__main__":
    ...
    async def receive_selected_option(request):
        data = await request.json()
        config.selected_option = data.get("selectedOption")
        print(f"Received selected option: {config.selected_option}")
        return web.json_response({"message": "Selected option received"})
    ...
    app.router.add_post("/api/selected-option", receive_selected_option)
    app.router.add_static('/', path='./static', name='static')
    web.run_app(app, host='localhost', port=8767)
